...
const GraphDrivers = (props) => {
  ...
  const [totalEarnMoney, setTotalEarnMoney] = useState(0);
  ...
  const fetchDrivingStatistic = async () => {
    ...
    earnMoneyData.splice(1); 

    const response = await fetch(
      "http://localhost:8080/api/drivers/getStatistics",
      {
        method: "POST",
        headers: {
    ...
    for (const driving of data) {
      ...
      earnMoneyData.push([formattedDate, driving.money]);
      totalEarnMoney += driving.money;

      averageSum = totalEarnMoney / totalNumberOfDrivings;
    }

    setTotalNumberOfDrivings(totalNumberOfDrivings);
    setTotalKilometersTravelled(totalKilometersTravelled);
    setTotalEarnMoney(totalEarnMoney);
    ...
    <div>
      ...
      <div className="graph">
        ...
        <div className="info">
          <label></label>
          <span className="label-space"></span>
          <label>Sum: {totalEarnMoney}</label>
          <span className="label-space"></span>
          <label>Average per driving: {averageSum}</label>
        </div>
      </div>
    </div>
  );
