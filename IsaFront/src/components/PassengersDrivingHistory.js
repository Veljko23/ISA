...
const PassengersDrivingHistory = (props) => {
  ...
  const fetchDrivingHistory = async () => {
    ...
    const response = await fetch(
      "http://localhost:8080/api/drivings/passengersDrivingHistory",
      {
        method: "POST",
        headers: {
  ...
  const drivingSelectClickHandler = (driving) => {
    props.setSelectedDriving(driving);
    navigate("/passengersDrivingDetailedView");
  };
          ...
          <table className="game-history-table">
            ...
            <tbody>
              {drivingHistory.map((driving) => (
                <tr onClick={() => drivingSelectClickHandler(driving)} key={driving.id}>
                  <td>{driving.start}</td>
                  <td>{driving.end}</td>
                  <td>{driving.price}</td>
...
export default PassengersDrivingHistory;